package CAITproject.CAIT.config;


import CAITproject.CAIT.Filter.JwtFilter;
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception { //we return the filter chain with the type httpsecurity
        ...
        return http
                ...
                .authorizeHttpRequests(request -> request.requestMatchers("/addUser","/login","/addGroup","/addQuestion")
                        .permitAll()
    ...
    public AuthenticationProvider authenticationProvider(BCryptPasswordEncoder passwordEncoder){ //this is an interface
        DaoAuthenticationProvider provider  = new DaoAuthenticationProvider(userDetailsService); //it's a class that implements the authenticationprovider
//        provider.setPasswordEncoder(NoOpPasswordEncoder.getInstance()); with no password encoder
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        config.setAllowedOrigins(List.of("http://localhost:3030")); // your React app origin
